 ...
 */
		...
		#define MOREINFO(x, y) do { std::cerr << x << y << std::endl; } while (0)
...
unsigned long millis() {
	...
	unsigned long millis = (unsigned long long)(tv.tv_sec) * 1000 + (unsigned long long)(tv.tv_usec) / 1000;
...
//	std::cerr << "spacing is " << static_cast<unsigned int>(spacing) << std::endl;
...
//	std::cerr << "start for analog pin ["<< static_cast<unsigned int>(pin) << "]/sample " << static_cast<unsigned int>(sample) << " is " << static_cast<unsigned int>(start) << std::endl;

//	std::cerr << "returning index " << static_cast<unsigned int>(getSampleOffset() + start) << " and index " <<  static_cast<unsigned int>(getSampleOffset() + start + 1) << ", val is " << static_cast<unsigned int>(getFrameData()[getSampleOffset() + start] << 8) <<  " and " <<  + static_cast<unsigned int>(getFrameData()[getSampleOffset() + start + 1]) << std::endl;
...
XBee::XBee(Stream &serial): _response(XBeeResponse()) {
        _pos = 0;
        _escape = false;
        _checksumTotal = 0;
        _nextFrameId = 0;
...
void XBee::readPacket() {
	...
	if (_response.isAvailable() || _response.isError()) {
		...
		INFO("DISCARDING PREVIOUS PACKET");
		resetResponse();
	}
        ...
        switch(_pos) {
			...
			default:
				...
				if (_pos > MAX_FRAME_DATA_SIZE) {
		        	...
		        	INFO("PACKET_EXCEEDS_BYTE_ARRAY_LENGTH");
					return;
				}
				...
				if (_pos == (_response.getPacketLength() + 3)) {
					...
					//std::cerr << "read checksum " << static_cast<unsigned int>(b) << " at pos " << static_cast<unsigned int>(_pos) << std::endl;
...
void XBee::send(XBeeRequest &request) {
	...
	//std::cerr << "frame length is " << static_cast<unsigned int>(request.getFrameDataLength()) << std::endl;

	for (int i = 0; i < request.getFrameDataLength(); i++) {
//		std::cerr << "sending byte [" << static_cast<unsigned int>(i) << "] " << std::endl;
...
//	std::cerr << "checksum is " << static_cast<unsigned int>(checksum) << std::endl;
...
//		std::cerr << "escaping byte [" << toHexString(b) << "] " << std::endl;
		write(ESCAPE);
		write(b ^ 0x20);
	} else {
		write(b);
